...
bool ImGui::tab(const char* icon, bool selected)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    float deltatime = 1.5f * GetIO().DeltaTime;
    ...
    it_hover->second = clamp(it_hover->second + (2.15f * GetIO().DeltaTime * (hovered ? 1.f : -1.f)), 0.0f, 0.0f);
    ...
    it_filled->second = clamp(it_filled->second + (2.15f * GetIO().DeltaTime * (selected ? 1.f : -1.5f)), it_hover->second, 1.f);
    it_filled->second *= GetStyle().Alpha;

    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), ImColor(29, 30, 34, int(255 * GetStyle().Alpha)), 8);
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 5, bb.Min.y + 4), ImColor(96, 97, 102, int(255 * GetStyle().Alpha)), icon);

    if (selected)
    {
        GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 5, bb.Min.y + 4), ImColor(155, 212, 90, int(255 * it_filled->second)), icon);
    }

    return pressed;
}

bool ImGui::TabEx(const char* icon, const char* label, const bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderTextClipped(bb.Min + ImVec2(36, 0), bb.Max - style.FramePadding, label, NULL, &label_size, ImVec2(0, 0.5), &bb);
...
bool ImGui::Tab(const char* icon, const char* label, const bool active, const ImVec2& size_arg)
{
...
bool ImGui::Bind(const char* label, int* key, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(GetWindowSize().x - size.x - style.ItemSpacing.x * 2, 0), window->DC.CursorPos + ImVec2(GetWindowSize().x - style.ItemSpacing.x * 2, size.y));
    ...
    it_hover->second = clamp(it_hover->second + (3.f * ImGui::GetIO().DeltaTime * ((hovered || user_clicked) ? 1.f : -1.f)), 0.0f, 1.f);
    ...
    RenderFrame(frame_bb.Min - ImVec2(1, 1), frame_bb.Max + ImVec2(1, 1), GetColorU32(ImGuiCol_FrameBgActive, it_hover->second), true, style.FrameRounding);
    RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);

    PushStyleColor(ImGuiCol_Text, it_hcolor->second);
    ...
    if (label_size.x > 0)
        RenderText(total_bb.Min + style.FramePadding, label);

    return value_changed;
}

bool ImGui::MultiCombo(const char* label, int current_item, const char* items[], int items_count, bool combos[])
{
    std::vector<std::string> vec;
    static std::string preview = "";
    auto returned = BeginCombo(label, preview.c_str());
    if (returned)
    {
        preview = "";
        for (size_t i = current_item; i < items_count; i++) {
            Selectable(items[i], &combos[i], ImGuiSelectableFlags_::ImGuiSelectableFlags_DontClosePopups);
            if (combos[i])
                vec.push_back(items[i]);
        }
        for (size_t i = current_item; i < vec.size(); i++) {
            if (vec.size() == 1)
                preview += vec.at(i);
            else if (!(i == vec.size() - 1))
                preview += vec.at(i) + ", ";
            else if (vec.size() >= 3)
                preview = vec.at(0) + ", " + vec.at(1) + ", ...";
            else
                preview += vec.at(i);
        }
        EndCombo();
...
void ImGui::Image_getforeg(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    if (border_col.w > 0.0f)
    {
        ImGui::GetForegroundDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(border_col), 0.0f);
        ImGui::GetForegroundDrawList()->AddImage(user_texture_id, bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), uv0, uv1, GetColorU32(tint_col));
    }
    else
    {
        ImGui::GetForegroundDrawList()->AddImage(user_texture_id, bb.Min, bb.Max, uv0, uv1, GetColorU32(tint_col));
    }
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect total_bb(pos, ImVec2(pos.x + 290, pos.y + 30));
    ...
    {
        ImGui::RenderFrame(total_bb.Min, total_bb.Max, ImColor(24, 25, 27), true, 5);
        window->DrawList->AddRectFilled(ImVec2(total_bb.Min.x + 265, total_bb.Min.y + 5), ImVec2(total_bb.Min.x + 285, total_bb.Min.y + 25), ImColor(47, 50, 55), 5);

        if (*v)
            RenderCheckMark(window->DrawList, { total_bb.Max.x - square_sz / 1.4f, total_bb.Min.y + square_sz / 3.1f }, ImColor(155, 212, 90), 10.f);

        if (label_size.x > 0.0f)
            window->DrawList->AddText(ImVec2(total_bb.Min.x + 10, total_bb.Min.y + 15 - ImGui::CalcTextSize(label).y / 2), ImColor(83, 86, 93), label);
    }
    ImGui::PopFont();
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    ImVec2 size = CalcItemSize(ImVec2(120, 20), expected_w, label_size.y + style.FramePadding.y * 2.0f);
    ...
    it_hover->second = clamp(it_hover->second + (3.f * ImGui::GetIO().DeltaTime * ((hovered || pressed || popup_open) ? 1.f : -1.f)), 0.0f, 1.f);

    RenderNavHighlight(frame_bb, id);

    window->DrawList->AddRectFilled(frame_bb.Min - ImVec2(1, 1), frame_bb.Max + ImVec2(1, 1), GetColorU32(ImGuiCol_FrameBgActive, it_hover->second), style.FrameRounding);
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderTextClipped(frame_bb.Min + style.FramePadding, frame_bb.Max, preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));
    if (label_size.x > 0)
        RenderText(total_bb.Min + style.FramePadding, label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const float w = CalcItemWidth();
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(10, 10), window->DC.CursorPos + ImVec2(window->Size.x - 60, 16 + label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 18), ImVec2(frame_bb.Max.x - 2, frame_bb.Max.y - 3 - 8), ImColor(40, 40, 40, int(255 * GetStyle().Alpha)), 4);
    ...
    {
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 18), ImVec2(grab_bb.Max.x - 2, grab_bb.Max.y - 3 - 8), ImColor(103 / 255.f, 151 / 255.f, 223 / 255.f, 0.9f * GetStyle().Alpha), 10);
        window->DrawList->AddRectFilled(ImVec2(grab_bb.Min.x - 2, grab_bb.Min.y + 6), ImVec2(grab_bb.Max.x, grab_bb.Max.y - 8), ImColor(150 / 255.f, 150 / 255.f, 150 / 255.f, 0.9f * GetStyle().Alpha), 3);
    }
    ...
    if (label_size.x > 0.0f)
        window->DrawList->AddText(ImVec2(window->DC.CursorPos.x + 9, window->DC.CursorPos.y - 32), ImColor(200 / 255.f, 200 / 255.f, 200 / 255.f, 1 * GetStyle().Alpha), label);
    Spacing();
...
bool ImGui::beginchildex(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    flags |= ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_ChildWindow;
    ...
    parent_window->DrawList->AddRectFilled(pos, p + size_arg, ImColor(19, 20, 22, int(255 * alpha)));
...
{
    ImGuiWindow* window = GetCurrentWindow();
    return beginchildex(str_id, window->GetID(str_id), size_arg, sub_tab, extra_flags);
}
